...
const ContentContainer = () => {
  ...
  if (ChatObject.activeChatId == "") {
    return (
      <div className="bg-gray-300 dark:bg-gray-700 w-screen hidden items-center justify-center md:flex">
        no active chat right now
      </div>
    );
  }
  return (
    <div className="flex-1  flex-col relative bg-gray-300 dark:bg-gray-700  overflow-y-auto hidden md:flex">
      {showProfile && (
      ...
      >
        {Loading ? (
          ...
          <div className="my-[20px]">
            {sortedChats && sortedChats.map((chat) => <MessageCard chat={chat} />)}
          </div>
        )}
      </main>
      <Input />
